...
* **數學原理**：
  ...
  其中 $Q_2(X)$ 為中位數，$Q_3(X) - Q_1(X)$ 為涵蓋中間 50% 資料的四分位距。這能確保防禦模型在特徵縮放時對異常訊號保持敏銳。
...
* **數學原理**：
  將包含 $k$ 種可能性的類別變數 $C$ 映射到一個 $k$ 維空間。對於第 $i$ 個類別，轉換後的向量 $v$ 定義為：
...
* **數學原理**：
  ...
  **B. Truncated SVD 降維**：
  利用奇異值分解 (SVD) 將稀疏矩陣 $A$ 降維至 $k$ 維空間，萃取潛在語意特徵：
...
* **採用演算法**：時間特徵萃取 (Datetime Extraction)。
* **數學原理**：
  時間單元 $t$ (如小時，週期 $T=24$) 映射到二維單位圓上以保持連續性：
...
* **數學原理**：
  ...
  **C. 感知亮度 (Perceived Luminance)**：
  將 RGB 統計量轉換為單一能量強度特徵 $L$：
