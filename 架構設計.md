...
### 2.2 目標欄偵測
如果未手動指定目標欄位，系統會依序檢查以下名稱：`target` $\rightarrow$ `label` $\rightarrow$ `class` $\rightarrow$ `y` $\rightarrow$ `c`，若皆未匹配，則預設使用最後一欄。
...
| 樣本規模 / 模式 | tabular_trials | n_repeats / n_seeds | use_kpca / use_kmeans | 額外優化 |
| :--- | :---: | :---: | :---: | :--- |
| **`--fast` 模式** | 5 | 1 / 1 | False | 最快速度，所有階段最小化 |
| **小資料（$< 500$ 筆）** | 30 | 2 / 3 | True | 加大重複 CV 與 seed 次數，抑制過擬合 |
| **標準資料（$< 50,000$ 筆）**| 20 | 1 / 1 | True | 標準搜尋，`top_k=2` 保留兩組超參 |
| **大資料（$\ge 50,000$ 筆）**| 10 | 1 / 1 | False | 關閉 KPCA 與 KMeans 以防記憶體與時間爆炸 |
...
- **PermutationExplainer**：若為非樹模型（如 MLP、CNN1D、Transformer 或 Stacking 集成模型），則自動退化（Fallback）至 `shap.PermutationExplainer`。
  - **效能取樣上限**：PermutationExplainer 的計算複雜度為 $O(\text{Samples} \times \text{Features} \times \text{Permutations})$，在大資料集上不加限制會耗時數小時。系統預設最多隨機取樣 **500 筆**測試樣本進行 SHAP 計算，將執行時間壓縮至數分鐘以內。
  - **max_evals 極限優化**：設定 `max_evals = 2 * n_features + 1`，這代表僅進行 1 輪 Permutation 擾動，比起預設約 4 輪，將計算速度提升 4 倍（特徵如 132 維時，時間可從 3.5 小時降至約 25 分鐘），且特徵重要性排名極其穩定，完全滿足視覺化精度需求。
...
### 10.3 DL 模型維度自適應對齊
在單獨調用 `generate_shap.py` 視覺化深度學習模型時，由於 PyTorch checkpoint (`.pt`) 記錄了訓練時特定的輸入特徵維度（`in_features`），若隨後建立的 `FeatureBuilder` 全域配置（如是否啟用 KMeans 距離或 Kernel PCA）不符，會因輸入維度不對導致 PyTorch Forward 傳播崩潰（`shape invalid` 錯誤）。
- **自動嘗試對齊**：系統在 `run_dl_viz()` 中，會自動依序嘗試四種常見的 `global_cfg` 組合（`use_kmeans` 與 `use_kpca` 的 True/False 開關組合）來擬合 `FeatureBuilder`，一旦得到的轉換特徵維度與 `in_features` 完全相符即停止搜尋，展現出高度的防呆與健壯性。

### 10.4 產出圖表 (以 scale=2 輸出高畫質 PNG)
...
- **Local Waterfall Plot (`{prefix}_waterfall.png`)**：
  針對第 0 個測試樣本（`sample_index=0`）展示其局部分析。採用 Plotly 的 **Waterfall 圖表**呈現，從基準期望值（Base Value / Expected Value，若為多類別輸出則自動展平取最後一類別的正類 log-odds 基準）出發，依影響力排序展示前 8 大特徵的貢獻度（正向貢獻以紅色柱狀圖表示，負向以藍色表示），最後收斂至該樣本的預測機率/值（總分以綠色表示），並自動根據數值大小調整小數點精度（絕對值 $< 0.01$ 時自動切換為 4 位小數，否則為 2 位小數），防止數值過小時顯示為 0.00。
- **Interaction Dependence Plot (`{prefix}_dependence.png`)**：
  ...
  - 使用商業常見的紅藍雙色漸層（`RdYlBu`）對點進行著色，揭示目標特徵與其他輔助特徵之間的非線性交互關係。�隱藏維度/通道數/Dropout）產生子代。
   - 回傳適應度分數最高的架構參數。
3. **訓練優化**：鎖定最佳 NAS 架構後，使用 `MLPTrainHPO` 或 `TSNetTrainHPO` 進行訓練超參數（如 `lr`, `dropout`）的調優。
*備註：為防止過擬合與時間浪費，當表格樣本數 $< 2000$ 且非時序模式時，系統會自動跳過 NAS，直接使用經驗預設架構。*
